...
echo "Alpine / MariaDB Client - Exporter"
echo "================================"
...
OUTPUT=$(eval mariadb-dump ${CMD} 2>&1)
exitcode=$?

if [[ $exitcode != 0 ]]; then echo $OUTPUT; exit $exitcode; fi

# Note: Ugly workaround here because `mariadb-dump` (unlike `mysql`) doesn't emit a proper exit code even in MySQL v8
# See https://bugs.mysql.com/bug.php?id=90538
if echo $OUTPUT | grep -qE '(.*)(mariadb-dump: Got error:|: eval:)(.*)'; then
    echo $OUTPUT
    exit 1
fi
